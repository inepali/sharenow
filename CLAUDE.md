...
2. **Supabase Edge Functions** (`supabase/functions/`) — R2 presigned URLs/deletes/stats, Stripe checkout + customer portal + subscription checks, WHCC print orders/pricing/product sync/webhook. Edge functions need the `R2_ACCOUNT_ID`/`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`/`R2_BUCKET_NAME` secrets set via `supabase secrets set`. Do NOT try to do image processing in an edge function — a 24MP photo exceeds the compute limits (`WORKER_RESOURCE_LIMIT`); that's why resizing happens in the browser.
