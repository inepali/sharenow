...
// Example: "https://sharemyshoot.com,https://www.sharemyshoot.com"
// Falls back to "*" only during local development.
...
// ── Health check (used by uptime monitors / deployment probes) ─────────────
...
// ── Supabase ───────────────────────────────────────────────────────────────
// Server uses SUPABASE_URL / SUPABASE_ANON_KEY (no VITE_ prefix).
// Falls back to VITE_* names for backward compatibility with the shared .env.
  ...
  try {
    ...
    if (source === "dropbox") {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        const mediumWebpBuffer = await sharp(buffer).resize({ width: 1200, height: 1200, fit: "inside", withoutEnlargement: true }).webp({ quality: 80 }).toBuffer();
        const thumbWebpBuffer = await sharp(buffer).resize({ width: 300, height: 300, fit: "cover" }).webp({ quality: 75 }).toBuffer();
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        // Generate previews
        const largeWebpBuffer = await sharp(buffer).resize({ width: 2000, height: 2000, fit: "inside", withoutEnlargement: true }).webp({ quality: 80 }).toBuffer();
        const mediumWebpBuffer = await sharp(buffer).resize({ width: 1200, height: 1200, fit: "inside", withoutEnlargement: true }).webp({ quality: 80 }).toBuffer();
        const thumbWebpBuffer = await sharp(buffer).resize({ width: 300, height: 300, fit: "cover" }).webp({ quality: 75 }).toBuffer();
